...
import { CALLOUT_CSS, renderArticleContent } from "@/lib/content/render";
...
export function MarkdownPreview({ content, className }: { content: string; className?: string }) {
  const html = useMemo(() => renderArticleContent(content), [content]);
  return (
    <div className={cn("callout-preview", className)}>
      {/* 与发布共用同一 CSS token，保证本地预览颜色 = 博客颜色 */}
