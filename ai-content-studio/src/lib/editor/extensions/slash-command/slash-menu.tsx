...
export const SlashMenu = forwardRef<SlashMenuRef, SlashMenuProps>(function SlashMenu(
  ...
  useImperativeHandle(ref, () => ({
    onDown: () => setSelectedIndex((i) => (i + 1) % Math.max(items.length, 1)),
    onUp: () => setSelectedIndex((i) => (i <= 0 ? Math.max(items.length - 1, 0) : i - 1)),
    onEnter: () => {
  ...
  return (
    ...
    >
      <div className="border-b px-3 py-1.5 text-xs text-muted-foreground">插入块</div>
      {items.length === 0 ? (
