...
vi.mock("@/lib/services/prompt-service", () => ({
  updatePrompt: updateMock,
  deletePrompt: deleteMock,
}));
vi.mock("@/lib/schemas/prompt", () => ({
  updatePromptSchema: { safeParse: schemaParse },
}));
...
describe("PUT /api/prompts/[id]", () => {
  ...
  it("Prompt 不存在返回 404", async () => {
    authMock.mockResolvedValue({ user: { email: "a@b.com" } });
    schemaParse.mockReturnValue({ success: true, data: { name: "Y" } });
    updateMock.mockRejectedValue(new Error("Prompt 不存在"));
    const res = await PUT(new Request("https://x", { method: "PUT", body: "{}" }), ctx("p1"));
    expect(res.status).toBe(404);
    expect(updateMock).toHaveBeenCalledWith("p1", { name: "Y" });
  });

  it("成功返回更新结果", async () => {
    authMock.mockResolvedValue({ user: { email: "a@b.com" } });
    schemaParse.mockReturnValue({ success: true, data: { content: "new" } });
    updateMock.mockResolvedValue({ id: "p1", version: 2 });
    const res = await PUT(new Request("https://x", { method: "PUT", body: "{}" }), ctx("p1"));
    expect(res.status).toBe(200);
    expect(updateMock).toHaveBeenCalledWith("p1", { content: "new" });
  });
