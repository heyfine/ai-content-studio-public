...
describe("POST /api/sources/[id]/refresh", () => {
  it("未授权 → 401", async () => {
    ...
    const r = await POST(
      new Request("http://localhost/api/sources/x/refresh", { method: "POST" }),
      ctx("x"),
    );
  ...
  it("刷新成功 → 200", async () => {
    ...
    mocks.refreshSource.mockResolvedValueOnce({
      source: { id: "s1" },
      created: false,
      versionBumped: true,
      versionNumber: 2,
    ...
    const r = await POST(
      new Request("http://localhost/api/sources/s1/refresh", { method: "POST" }),
      ctx("s1"),
    );
